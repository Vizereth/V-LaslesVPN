(() => {
    ...
    class PlanCard {
        ...
        onClickHandler(e) {
            ...
            if (this.currentCard) this.deselectCard();
...
***************************************************************************** */
    ...
    function scroll_scrollTo(el, options = {}) {
        return __awaiter(this, void 0, void 0, (function*() {
            if (!(el instanceof Element) && !(el instanceof Window)) throw new Error(`element passed to scrollTo() must be either the window or a DOM element, you passed ${el}!`);
            ...
            const scroll = (from, to, prop, startTime, duration = DEFAULT_DURATION, easeFunc, callback) => {
                window.requestAnimationFrame((() => {
                    ...
                    if (from === to) return callback ? callback() : null;
                    setScrollPosition(el, easeFunc(time) * (to - from) + from);
                    if (time < 1) scroll(from, to, prop, startTime, duration, easeFunc, callback); else if (callback) callback();
                }));
            };
            const currentScrollPosition = getScrollPosition(el);
            const scrollProperty = getScrollPropertyByElement(el);
            return new Promise((resolve => {
                scroll(currentScrollPosition, "number" === typeof options.top ? options.top : currentScrollPosition, scrollProperty, Date.now(), options.duration, getEasing(options.easing), resolve);
            }));
        }));
    }
    function scrollIntoView(element, scroller, options) {
        ...
        return scroll_scrollTo(scroller, {
    ...
    function validateElement(element) {
        ...
        if (!(element instanceof HTMLElement)) throw new Error(`The element passed to scrollIntoView() must be a valid element. You passed ${element}.`);
    }
    function getScrollPropertyByElement(el) {
        ...
        const axis = "y";
        if (el instanceof Window) return props.window[axis]; else return props.element[axis];
    }
    function sanitizeScrollOptions(options = {}) {
        if ("smooth" === options.behavior) {
            options.easing = "ease-in-out";
            options.duration = DEFAULT_DURATION;
        }
        if ("auto" === options.behavior) {
            options.duration = 0;
    ...
    function getScrollPosition(el) {
        const document = utils.getDocument();
        if (el === document.body || el === document.documentElement || el instanceof Window) return document.body.scrollTop || document.documentElement.scrollTop; else return el.scrollTop;
    }
    ...
    (() => {
        ...
        for (const key in storage) {
            ...
            if (linkEl) linkEl.addEventListener("click", (() => {
                scrollIntoView(scrollToEl, document.body, {
                    behavior: "smooth"
                });
            }));
            if (linkMobileEl) linkMobileEl.addEventListener("click", (() => {
                scrollIntoView(scrollToEl, document.body, {
                    behavior: "smooth"
                });
            }));
